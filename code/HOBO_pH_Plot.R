...
MaypH <- read.csv("sensor data/processed/processed_May2023_pH.csv")
JulypH <- read.csv("sensor data/processed/processed_July2023_pH.csv")


allpH <- rbind(MaypH, JulypH) %>%
...
allpH %>%
  filter(result_value > 6.4 & result_value < 8) %>%
  ggplot(aes(x=Date_formatted,y=temp, color=tidal)) +
